<div x-data="{ mobileFilterDialogOpen: false }">
                        ...
                        <fieldset>
                            <legend class="w-full px-2">
                                ...
                                <button type="button"
                                    ...
                                    <span class="text-sm font-medium text-gray-900">{{ word('neo.color') }}</span>
                            ...
                            <div class="px-4 pb-2 pt-4" id="filter-section-0" x-show="filterOpened">
                                <div class="space-y-6">
                                    @if ($this->filters['color'])
                                        <button wire:click="resetFilter('color')"
                                                class="text-sm text-gray-600 underline">
                                            {{ word('neo.reset_filter') }}
                                        </button>
                                    @endif
                                    @foreach($this->getFilterOptions('color') as $color)
                                        <div class="flex items-center">
                                            <input id="color-{{ $color }}-mobile" wire:model.live="filters.color"
                                                   value="{{ $color }}" type="checkbox"
                                                   class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                            <label for="color-{{ $color }}-mobile"
                                                   class="ml-3 text-sm text-gray-500">{{ $color }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <fieldset>
                            <legend class="w-full px-2">
                                ...
                                <button type="button"
                                        ...
                                        class="flex w-full items-center justify-between p-2 text-gray-400 hover:text-gray-500"
                                        aria-controls="filter-section-0" aria-expanded="false">
                                    <span class="text-sm font-medium text-gray-900">{{ word('neo.size') }}</span>
                                    <span class="ml-6 flex h-7 items-center">
                                              <svg class="h-5 w-5 transform"
                                                   :class="{ '-rotate-180': filterOpened, 'rotate-0': !filterOpened }"
                                                   viewBox="0 0 20 20" fill="currentColor"
                            ...
                            <div class="px-4 pb-2 pt-4" id="filter-section-0" x-show="filterOpened">
                                <div class="space-y-6">
                                    @if ($this->filters['size'])
                                        <button wire:click="resetFilter('size')"
                                                class="text-sm text-gray-600 underline">
                                            {{ word('neo.reset_filter') }}
                                        </button>
                                    @endif
                                    @foreach($this->getFilterOptions('size') as $size)
                                        <div class="flex items-center">
                                            <input id="size-{{ $size }}-mobile" wire:model.live="filters.size"
                                                   value="{{ $size }}" type="checkbox"
                                                   class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                            <label for="size-{{ $size }}-mobile"
                                                   class="ml-3 text-sm text-gray-500">{{ $size }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
    ...
    <main class="mx-auto max-w-2xl px-4 lg:max-w-7xl lg:px-8">
        <div class="border-b border-gray-200 pb-10 pt-10">
            @if ($breadcrumbs = $page->getBreadcrumbsArray($locale) and count($breadcrumbs) > 0)
        ...
        <div class="pb-24 pt-12 lg:grid lg:grid-cols-3 lg:gap-x-8 xl:grid-cols-4">
            <aside>
                ...
                <button x-on:click="mobileFilterDialogOpen = true" type="button"
                    ...
                    <span class="text-sm font-medium text-gray-700">{{ word('neo.filters') }}</span>
                ...
                <div class="hidden lg:block">
                    <form class="space-y-10 divide-y divide-gray-200">
                        ...
                        <div class="{{ $categories->count() > 0 ? 'pt-10' : '' }}">
                            <fieldset>
                                <legend class="block text-sm font-medium text-gray-900">{{ word('neo.color') }}</legend>
                                    ...
                                    @foreach($this->getFilterOptions('color') as $color)
                                        <div class="flex items-center">
                                            <input id="color-{{ $color }}"
                                                   ...
                                                   type="checkbox"
                                                   class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                            <label for="color-{{ $color }}" class="ml-3 text-sm text-gray-600">
                                                {{ $color }}
                                            </label>
                                        </div>
                                    @endforeach
                        ...
                        <div class="pt-10">
                            <fieldset>
                                <legend class="block text-sm font-medium text-gray-900">{{ word('neo.size') }}</legend>
                                <div class="space-y-3 pt-6">
                                    @if ($this->filters['size'])
                                        <button wire:click="resetFilter('size')"
                                                class="text-sm text-gray-600 underline">
                                    ...
                                    @foreach($this->getFilterOptions('size') as $size)
                                        <div class="flex items-center">
                                            <input id="size-{{ $size }}"
                                                   ...
                                                   type="checkbox"
                                                   class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                            <label for="size-{{ $size }}" class="ml-3 text-sm text-gray-600">
                                                {{ $size }}
                                            </label>
                                        </div>
                                    @endforeach
                                ...
                                <div class="flex flex-1 flex-col justify-end">
                                    @if ($product->getSpecification('color'))
                                        <p class="text-sm italic text-gray-500">
                                            {{ word('neo.color') }}
                                            : {{ $product->getSpecification('color')->value }}</p>
                                    @endif
                                    @if ($product->getSpecification('size'))
                                        <p class="text-sm italic text-gray-500">
                                            {{ word('neo.size') }}: {{ $product->getSpecification('size')->value }}</p>
                                    @endif
                                    @if ($product->getPrice())
